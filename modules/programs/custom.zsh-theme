...
prompt_git() {
  ...
  if [[ "$(git rev-parse --is-inside-work-tree 2>/dev/null)" = "true" ]]; then
    ...
    if [[ -n $dirty ]]; then
      echo -n "\e[1;33m" # set FG color to yellow
    else
      echo -n "\e[1;36m" # set FG color to cyan
    fi
    echo -n "${${ref:gs/%/%%}/refs\/heads\//}${vcs_info_msg_0_%%}${mode}"
    echo -n "\e[0m" # reset color
    echo -n "]"
  fi
}
...
build_prompt() {
  ...
  [[ $RETURN_VALUE -ne 0 ]] && echo -n "(\e[1;31mX%?\e[0m)"
